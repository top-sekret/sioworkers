...
class PRootExecutor(BaseExecutor):
    ...
    def __init__(self, sandbox):
        ...
        if sandbox.endswith('amd64') or sandbox == 'compiler-gcc.12_2_0' or sandbox == 'compiler-gcc.14_2_0' or sandbox.startswith('compiler-nasm') or sandbox.startswith('compiler-gas') or sandbox.startswith('compiler-cipp'): # TEMPFIX
...
class IsolateExecutor(BasicIsolateExecutor):
    ...
    def _execute(self, command, **kwargs):
        ...
        if kwargs['stdout'] is None:
            self._write_all_to_fd(kwargs['stderr'], open(os.path.join(self.isolate_root, self.out_filename), 'r').read())
        else:
